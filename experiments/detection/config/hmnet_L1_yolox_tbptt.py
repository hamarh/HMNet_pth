...
DELTA_T = 5e3
SEGMENT_DURATION = 810e3
N_SEGMENTS = 10
TRAIN_DURATION = SEGMENT_DURATION * N_SEGMENTS
INPUT_SIZE = (240, 304)
...
class TrainSettings(object):
    ...
    def get_model(self):
        ...
        model = HMDet(backbone, neck, head, devices=[d0,d1])
        model.init_weights()
        return model
    ...
    resume      = ''
    print_freq  = 1
    load        = './workspace/hmnet_L1_yolox/checkpoint.pth.tar'
...
class TestSettings(object):
    def get_model(self, devices=None, speed_test='single_process'):
        ...
        if speed_test == 'multi_process':
            model.backbone.set_multi_process(False)
        elif speed_test == 'cuda_stream':
            model.backbone.set_cuda_stream(True)
